...
<link rel="stylesheet" href="<%=basePath %>iot/css/video.css">
<script src="<%=basePath %>webdemo/My97DatePicker/WdatePicker.js"></script>
<script src="<%=basePath %>webdemo/json2.js"></script>
<link rel="stylesheet" href="<%=basePath %>iot/css/video.css">
...
<title>实时视频</title>
<link rel="stylesheet" type="text/css" href="<%=basePath %>webdemo/DSSPlay.css"/>
<script src="<%=basePath %>webdemo/ocxfun.js"></script>

</head>
<body id="mbody" onload="init()" onunload="ButtonLogout_onclick()">

<div id="mdiv" style="position: relative;left:340px;'" >

</div>
<div id="mainbg">
<input type="hidden" value="<%=basePath %>"  class="basePathValue">
<!-- 左边 -->
		<div class="video_left" style="float:left">
			<div class="zzsc-container">
				<div class="content">
					<div id="jquery-accordion-menu"
						class="jquery-accordion-menu red video_all"
						style="max-height: 600px; ">
						<!--  搜索条件 -->
						<div id="search_div">
							<div >
								<select class="form-control" style="width: 130px;" id="baseId"
									onchange="loadMassifAddress()"></select>
							</div>
							<div
								style="position: relative; left: 131px; top: -40px;">
								<select class="form-control" style="width:130px;" id="massifId"></select>
							</div>
							<div class="jquery-accordion-menu-header" id="form_video"
								style="position:relative;top:-29px"></div>
						</div>
						<ul id="demo-list_video">
							<li class="active"><a href="#">
									<div class="video_fa_tit">Events</div>
									<div class="video_fa">
										<img src="<%=basePath %>iot/img/video_tie.png" alt="">
									</div>
							</a></li>
							<li><a href="#">
									<div class="video_fa_tit">Events</div>
...
<!-- 右边 -->
		<div class="video_right">
			...
			<!-- 控件 -->	
				<div id="main"  style="position:relative;top:-20px;">
				    <div style="width: 90%;height: 30px;overflow: auto;display: none" id="RetInfo">
					...
					<div style="width: 90%;height: 30px;overflow: auto;display: none" id="GlobalInfo">
				 	...
				 	<div id="mviewl" style="position:absolute;top:0px;">
						<div id="obj"  style="width:1700px;height:1200px">
...
<div class="rightDiv" style="width:50%;position:absolute;left:1200px;]">
	<div style="border:1px solid white;padding-top:5px;">
    	通道ID：<input type="text" id="textCameraID" value="1000000$1$0$0" />
	</div>
	<ul id="tab">
	...
	<div style="border:1px solid white;padding:5px;">
		<div style="width:50%; float:left">
			登陆登出</br>
			IP：   &nbsp;&nbsp;&nbsp;&nbsp;  &nbsp;&nbsp;<input type="text" id="textIP" value="192.168.89.245"  size="10"/></br>
			端口：  &nbsp;&nbsp; <input type="text" id="textPort" value="9000" size="10"/></br>
			用户名：&nbsp;<input type="text" id="textUser" value="dzVideoUser" size="10"/></br>
			密码： &nbsp;&nbsp;&nbsp;<input type="text" id="textPassword" value="developer" size="10"/></br></br>
	...
	<div style="border:1px solid white;padding-top:20px;">
		<div style="margin-left: 10px;">
			...
			按窗口号
				...
				<input type="button" id="buttonStartBackTimePlaybackByWndNo" value="按时间倒放" onclick="return ButtonStartBackTimePlaybackByWndNo_onclick()" />
		    ...
		    <br /><br />
			下载进度：<input type="text" id="textDownloadProgress" value=""  size="5" />

                        <br /><br />图片：
			<br />
                         开始时间：<input id="textStartTimePicture" type="text" name="" value='2013-10-17 00:00:00' onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',startDate:'%y-%M-%d 00:00:00',maxDate:'%y-%M-%d',minDate:'1970-01-01 10:00:00'})" class="select-text" style="padding: 0 5px;" readonly/>			  
			结束时间：<input id="textEndTimePicture" type="text" name="" value='2013-10-17 23:59:59' onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'%y-%M-%d',minDate:'1970-01-01 10:00:00'})" class="select-text" style="padding: 0 5px;" readonly/>
 
			<input type="button" id="ButtonQueryPictureRecord" value="查询图片" onclick="return ButtonQueryPictureRecord_onclick()" />

                         <br />删除：
                         <br />
                          图片路径：<input type="text" id="textPicturePath" value="" />
                          <input type="button" id="ButtonDeletePictureRecord" value="查询图片" onclick="return ButtonDeletePictureRecord_onclick()" />
			<div style="border:1px solid white;padding:5px;">
			IP:<input type="text" id="textJDIP2" value="" size="8" />
			端口：<input type="text" id="textJDPort2" value="" size="5" />
			历史视频ID：<input type="text" id="textJDFileId" value="" size="5" />
			访问令牌：<input type="text" id="textJDToken2" value="" size="5" />
		...
		<div style="margin-left: 10px;">
			...
			<table >
				...
				<tr>
					...
					<td align="center"><input type="button" id="" value="右下" onclick="return ButtonAsyncPtzDirection_onclick(8)" /></br></td>
				</tr>
			</table>
	...
	<div style="border:1px solid white;padding-top:5px;m">
		<div style="margin-left: 10px;">
		X:<input type="text" id="textXPosition" value="" size="3" />
		Y:<input type="text" id="textYPosition" value="" size="3" />
		变倍:
	...
	<div style="margin-left: 10px;">
		X(1~128):<input type="text" id="textXPositionEX" value="25" size="3" />
		Y(1~128):<input type="text" id="textYPositionEX" value="25" size="3" />
		变倍(1~128):<input type="text" id="selectPtzSitStepEX" value="3" size="3" />
		Xspeed(-1~1):<input type="text" id="textXspeedEX" value="0.2" size="3" />
		Yspeed(-1~1):<input type="text" id="textYspeedEX" value="0.2" size="3" />
		Zspeed(-1~1):<input type="text" id="textZspeedEX" value="0.2" size="3" />
		<input type="button" id="buttonPtzSitEX" value="设置球机云台参数" onclick="return ButtonSitPtzParam_onclick()" /><br /><br />
		定位类型:
	...
	预置点编号:<select id="selectPtzLockCamera" >
	...
	<div style="border:1px solid white;padding-top:5px;m">
			<div style="margin-left: 10px;">
				预置点编号:<input type="text" id="textPrePointNum" value=""  size="5" />
				预置点名称:<input type="text" id="textPrePointName" value=""  size="5" />
	...
	<div style="border:1px solid white;padding-top:5px;m">
			...
			<input type="button" id="buttonQueryChannelViewInfo" value="查询球机可视域" onclick="return ButtonQueryChannelViewInfo_onclick()" />
			设备ID:<input type="text" id="viewinfoDeviceID" value="1000028"  size="10" />
	...
	<div style="border:1px solid white;padding:20px;">
		...
		采样位数：<select id="selectBitType" >
				<option value="8">8</option>
				<option value="16">16</option>
		</select></br>
		采样频率：<select id="selectSampleType" >
				<option value="8000">8000</option>
				<option value="16000">16000</option>
				<option value="8192">8192</option>
				<option value="48000">48000</option>
		</select></br>
		传输类型：<select id="selectTransType" >
	...
	<div style="border:1px solid white;padding:5px;">
		screenId(从电视墙布局xml中获取):<input type="text" id="textOpenWndScreenId" value="" size="5" /></br>
		左边距：<input type="text" id="textOpenWndLeft" value="" size="5" /></br>
		上边距：<input type="text" id="textOpenWndTop" value="" size="5" /></br>
		窗口宽度：<input type="text" id="textOpenWndWidth" value="" size="5" /></br>
		窗口高度：<input type="text" id="textOpenWndHeight" value="" size="5" /></br>
		<input type="button" id="ButtonOpenWindow" value="开窗" onclick="return OpenWindow()" />
		<input type="button" id="ButtonClearScreen" value="清屏" onclick="return WallClean()" />
	</div>
	<div style="border:1px solid white;padding:5px;">
	...
	<div style="border:1px solid white;padding:20px;">
		...
		控制命令:<select id="VideoHostcontrolType" >
...
<script src="<%=basePath %>iot/js/video.js" type="text/javascript"></script>
</div>
</body>
</html>