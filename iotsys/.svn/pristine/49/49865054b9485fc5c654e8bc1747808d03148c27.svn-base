...
(function ($) {
    $.fn.hsCheckData = function (options) {
        ...
        $(this).click(function () {
            ...
            //2、数据列表
            var dataListHtml = dataListHtmlFun(0);
            //3、确定按钮
            ...
            $("[name='datali']>div").click(function () {
                ...
                if (!opts.isShowCheckBox) {
                    ...
                    $('#' + id).parent().next('input[type="hidden"]').val(data_id);
                    var me = opts.method;
                    me();
                    $('#' + id + "_hcd").remove();
                    $('#' + id).removeClass('hsCjeckData_check');
                ...
                $("#" + id + "_btn").click(function () {
                    ...
                    $('#' + id + '_hcd').find('input[type="checkbox"]').each(function (index, element) {
                        if ($(this).prop("checked")) {
                            ...
                            //封装id+value的值
                            data_idValue +="'"+$(this).val()+"':'"+ $(this).nextAll("span").text()+"'" + ",";
                        }
                    });
                    ...
                    //取得id+value的值
                    $('#' + id).attr('data-idValue', data_idValue);
                    console.log($('#' + id));
                    $('#' + id).attr('data-id', data_id).text(data_value);
                    $('#' + id).parent().next('input[type="hidden"]').val(data_id);
                    var me = opts.method;
            ...
            $('#' + id + '_input').keyup(function () {
                if ($(this).val() != "") {
                    ...
                    $('#' + id + "_hcd").children(".hcd_dataList").html(dataListHtmlFun(1));
                    setCheckData();
                }
            });
            return false;
        ...
        function filterHtmlFun() {
            ...
            html += "<input type='text' id='" + id + "_input' class='hcd_filter_input' placeholder='请输入关键字'/>";
            html += "</div>";
            return html;
        }
        //数据列表
        function dataListHtmlFun(num) {
        ...
        function setCheckData() {
            if ($('#' + id).attr("data-idvalue") != undefined && $('#' + id).attr("data-idvalue") != "") {
                var data_id = $('#' + id).attr("data-idvalue");
                var dataidArray = data_id.split(',');
                for (var i = 0; i < dataidArray.length; i++) {
                	var a = dataidArray[i].split(':');
                	if (a.length==2) {
                		$('#' + id + "_hcd").find('.hcd_dataList input[value=' + a[0] + ']').prop("checked", true);
					}
                }
            }
        }
