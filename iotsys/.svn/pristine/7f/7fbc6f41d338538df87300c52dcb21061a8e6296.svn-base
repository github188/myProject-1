...
SWFUpload.prototype.cleanUp = function(f) {
    try {
        if (this.movieElement && typeof(f.CallFunction) === "unknown") {
            ...
            for (var h in f) {
                try {
                    if (typeof(f[h]) === "function" && f[0] >= 'A' && f[0] <= 'Z') {
                        f[h] = null;
                    }
                } catch(e) {}
            }
            ...
            if (c.inArray("onUploadComplete", g.overrideEvents) < 0) {
                if (g.removeCompleted) {
                    ...
                    case SWFUpload.FILE_STATUS.COMPLETE:
                        setTimeout(function() {
                            if (c("#" + f.id)) {
                    //************* update by shenyan 2016-09-02 ***********
                            	//获取上传时显示的那个进度条的框的id
                            	var complateDivId=c("#" + f.id).parent().attr("id");
                            	//获取文件上传按钮的id
                            	var fileBtnId=c("#"+complateDivId).siblings().first().attr("id");
                                ...
                                function() {
                                	//将文件上传的按钮启用
                                	c("#"+fileBtnId).uploadify('disable', false);
                                    c(this).remove();
                                });
                            }
                        },
